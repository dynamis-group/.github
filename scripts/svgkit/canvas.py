...
class Canvas:
    ...
    def wrap(self, content: str, style: str, width: float, *, balance: bool = True) -> list[str]:
        """Word wrap to ``width`` px with the embedded font's real advances.

        With ``balance`` the lines are evened out (like CSS ``text-wrap: balance``), so a
        paragraph never ends on an orphaned word.
        """
        lines = self._greedy(content, style, width)
        if not balance or len(lines) < 2:
            return lines
        low, high = 0.0, width
        for _ in range(24):
            middle = (low + high) / 2
            if len(self._greedy(content, style, middle)) == len(lines):
                high = middle
            else:
                low = middle
        return self._greedy(content, style, high)

    def _greedy(self, content: str, style: str, width: float) -> list[str]:
        lines: list[str] = []
        current = ""
